#!/bin/bash
# Post-3,200,000,000-event runbook: check -> snapshot -> validate -> migrate -> unfreeze.
#
...
#
#   ./finish_property_event_comparison.sh check      # read-only; safe at any time
#   ./finish_property_event_comparison.sh snapshot   # needs: 3,200,000,000-event genuinely complete
#   ./finish_property_event_comparison.sh validate   # needs: .snapshot_complete
#   ./finish_property_event_comparison.sh migrate    # needs: .validated  (+ manifest digest match)
#   ./finish_property_event_comparison.sh unfreeze   # needs: .migrated
#   ./finish_property_event_comparison.sh all        # the five above, stopping at the first failure
#   ./finish_property_event_comparison.sh snapshot-recovery   # snapshot an INCOMPLETE attempt
#
...
# --------------------------------------------------------------------------
# check -- is the 3,200,000,000-event confirmation genuinely COMPLETE?
#
...
# --------------------------------------------------------------------------
check() {
  say "checking whether the 3,200,000,000-event confirmation is COMPLETE (not merely terminal)"
  DATABASE="$DATABASE" EVENTS_3200000000_TRIAL="$EVENTS_3200000000_TRIAL" EVENTS_3200000000_RESULT_JSON="$EVENTS_3200000000_RESULT_JSON" \
  EVENTS_3200000000_OWNER_PID="${EVENTS_3200000000_OWNER_PID:-}" $PY - <<'PY'
import json, os, sqlite3, sys, time
database, trial = os.environ["DATABASE"], os.environ["EVENTS_3200000000_TRIAL"]
result_json = os.environ["EVENTS_3200000000_RESULT_JSON"]
GOOD = ("success", "success_zero_qp")
SUBRUN_GOOD = ("success", "success_zero_qp", "teardown_sigsegv_complete")
fail = []
c = sqlite3.connect(f"file:{database}?mode=ro", uri=True); c.row_factory = sqlite3.Row
r = c.execute("SELECT * FROM trials WHERE trial_id=?", (trial,)).fetchone()
if r is None:
    print(f"  {trial}: not in the database"); sys.exit(2)
...
pid = os.environ.get("EVENTS_3200000000_OWNER_PID", "").strip()
if pid:
    ...
    if procs is None:
        ...
        fail.append("cannot confirm the writer exited (set EVENTS_3200000000_OWNER_PID to assert it)")
    elif procs:
...
PY
  rc=$?
  case $rc in
    0) say "3,200,000,000-event is COMPLETE -- safe to snapshot" ;;
    2) say "3,200,000,000-event trial not found in the database" ;;
    *) say "3,200,000,000-event is NOT complete -- do NOT snapshot, migrate or unfreeze.
...
_do_snapshot() {
  ...
  # unreachable.
  [ -f "$SNAPDIR/.snapshot_complete" ] || [ -f "$SNAPDIR/$(basename "$DATABASE")" ] \
  ...
  DATABASE="$DATABASE" DEST="$SNAPDIR/$(basename "$DATABASE")" $PY - <<'PY' || die "backup failed"
import os, sqlite3
src = sqlite3.connect(f"file:{os.environ['DATABASE']}?mode=ro", uri=True)
...
PY
  ...
  printf 'mode=%s\ncreated_at=%s\ndatabase=%s\nhost=%s\nmanifest_sha256=%s\n' \
    "$mode" "$(date '+%F %T')" "$DATABASE" "$(hostname)" \
...
# first version of this runbook conflated them: `snapshot` demanded that the 3,200,000,000-event
...
# the database be migrated. A gate that can never be satisfied is not fail-closed,
# it is stuck.
no_live_writer() {
...
# writer. An open file descriptor on the database does not lie.
...
if not ns_ok:
    ...
    print(f"  {database} before snapshotting or migrating.")
    sys.exit(4)
...
try:
    ...
    self_pid = os.getpid()
targets = {os.path.realpath(database), os.path.realpath(database + "-wal"),
           os.path.realpath(database + "-shm")}
...
print(f"  holding the database open   : {len(holders)}")
for h in holders[:5]:
    print(f"    {h}")
if not scanned:
...
snapshot() {
  ...
  if check; then
    ...
    say "3,200,000,000-event did not succeed, but the campaign was explicitly closed:"
    sed -n 's/^reason=/  reason: /p' "$SNAPDIR/.campaign_closed"
    _do_snapshot closed_incomplete
  else
    die "The 3,200,000,000-event confirmation did NOT complete successfully.
           ...
           not be labelled a completed confirmation. Record the decision:
             ...
             ./finish_property_event_comparison.sh close-incomplete 'why it will not be retried'
...
close_incomplete() {
  ...
  [ -n "$reason" ] || die "close-incomplete needs a reason, e.g.
           ./finish_property_event_comparison.sh close-incomplete 'best_random timed out at 41.7 h'"
  no_live_writer || die "a writer may still be live; refusing to close the campaign"
  mkdir -p "$SNAPDIR" || die "cannot create $SNAPDIR"
  {
    printf 'closed_at=%s\n' "$(date '+%F %T')"
    printf 'host=%s\n' "$(hostname)"
    printf 'trial=%s\n' "$EVENTS_3200000000_TRIAL"
    printf 'reason=%s\n' "$reason"
...
validate() {
  ...
  DATABASE="$DATABASE" SNAP="$SNAPDIR/$(basename "$DATABASE")" $PY - <<'PY' || die "snapshot invariants FAILED"
import json, os, sqlite3, sys
a = sqlite3.connect(f"file:{os.environ['DATABASE']}?mode=ro", uri=True); a.row_factory = sqlite3.Row
...
PY
  ...
  $AUDIT_CMD --database "$SNAPDIR/$(basename "$DATABASE")" \
...
migrate() {
  ...
  say "migrating the LIVE database (additive columns only; no row is rewritten)"
  DATABASE="$DATABASE" HIST="$SNAPDIR/$(basename "$DATABASE").statushist.json" \
...
with Database(database, allow_frozen=True) as led:          # migration runs on open
...
PY
  ...
  printf 'migrated_at=%s\nhost=%s\ndatabase=%s\nvalidated_manifest=%s\n' \
    "$(date '+%F %T')" "$(hostname)" "$DATABASE" "$current" \
...
unfreeze() {
  ...
  printf 'unfrozen_at=%s\nhost=%s\n' "$(date '+%F %T')" "$(hostname)" \
    > "$SNAPDIR/.unfrozen"
  say "database unfrozen; the reason is preserved at $SNAPDIR/frozen.reason.txt"
}
...
status() {
  ...
  [ -f "$FREEZE" ] && say "database is FROZEN" || say "database is writable"
}
