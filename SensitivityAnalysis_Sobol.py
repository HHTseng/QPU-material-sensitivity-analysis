...
electrode_params = [
("/main/gun/setEnergy ", 0.000382, [0.000382*lbf,0.000382*ubf], " eV"),
...
("/g4cmp/temperature ", 0.0, [0.0, 0.05], " K"),
...
sampler = qmc.Sobol(d=VNUM, scramble=False, seed = 4)
samples = sampler.random_base2(m=14)
samples = np.round(qmc.scale(samples, lowerbound, upperbound), 5)
...
with open(os.path.join(RESULTS_DIR, "SobolSequence.csv"), 'w', newline='') as csvfile:
...
def generate_runfiles(samp,samp_No):
    ...
    sample_name = "Sobol_" + str(samp_No)
    ...
    lines = replace_line(lines,"/main/detector_param/setSubstrateName ","/main/detector_param/setSubstrateName Sobol_" + str(samp_No))
...
if __name__ == "__main__":
    ...
    for s, samp in enumerate(samples):
        ...
        macro_files.append(os.path.join(MACROS_DIR, "Sobol_" + str(s) + ".mac"))
