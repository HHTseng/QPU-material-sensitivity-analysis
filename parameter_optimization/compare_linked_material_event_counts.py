...
and 1e8? If it does, the cheap event_count is a valid screen. If it does not, every
ranking claim made at the cheap event_count has to be re-made.

Reports, per event_count:

  * Spearman rho and Kendall tau against the baseline event_count's ranking;
  ...
  * whether the top-1 and top-3 sets are preserved (what actually matters for
    selection);
  * per-candidate QPs-per-primary-event, which is the event_count-independent quantity;
  * pairs that were statistically unresolved at the baseline event_count and whether
    the higher event_count resolves them.
...
the interface model, not by Poisson noise -- so agreement between event_counts means
"the counting noise was not the problem", never "the ranking is correct".

Usage:
    python compare_linked_material_event_counts.py \
        --baseline-database stage3_trials.sqlite \
        --database e7=stage3_trials_e7.sqlite \
        --database e8=stage3_trials_e8.sqlite
"""
...
def load(path, events_total=None):
    ...
    `events_total` pins the event_count. Without it a database holding more than one event
    ...
    wrong baseline. A name may also legitimately repeat within one event_count when the
...
def main():
    ...
    ap.add_argument("--baseline-database", default=os.path.join(HERE, "stage3_trials.sqlite"))
    ...
    ap.add_argument("--baseline-events", type=int, default=4000000,
                    help="events_total that identifies the baseline event_count")
    args = ap.parse_args()

    base = load(args.baseline_database, events_total=args.baseline_events)
    if not base:
        sys.exit(f"no scored trials in {args.baseline_database}")

    event_counts = []
    for spec in args.database:
        if "=" not in spec:
            sys.exit(f"--database needs label=path, got {spec!r}")
        label, path = spec.split("=", 1)
        rows = load(path)
        if rows:
            event_counts.append((label, path, rows))
        else:
            print(f"(event_count {label}: no scored trials yet at {path} -- skipped)")
    if not event_counts:
        sys.exit("no higher-statistics event_count has scored trials yet; nothing to compare.")

    per_sub_base = None
    any_row = next(iter(base.values()))
    per_sub_base = any_row["events_total"] // (any_row["n_positions"] * any_row["n_replicas"])
    print(f"\nBaseline event_count {args.baseline_label}: {len(base)} candidates, "
          f"{per_sub_base:,} events per sub-run\n")

    base_rank = ranks(base)

    for label, path, rows in event_counts:
        shared = sorted(set(base) & set(rows))
        if len(shared) < 2:
            print(f"event_count {label}: only {len(shared)} shared candidate(s); skipping.")
            continue
        ...
        print("=" * 78)
        print(f"EVENT_COUNT {label}: {per_sub:,} events per sub-run "
        ...
        # Pairs the baseline event_count could not order.
    ...
    print("-" * 78)
    print("Reminder: agreement between event_counts shows counting noise was not the")
