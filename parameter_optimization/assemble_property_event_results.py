#!/usr/bin/env python3
"""Assemble the combined 3,200,000,000-primary table. NO simulation.

    python assemble_property_event_results.py
    python assemble_property_event_results.py --compare-primary-counts

Replaces steps 4 and 5 of `run_property_event_comparison.sh`. Those steps re-invoked
...
interpretation has not changed. It would also open a new `Database`, running the
...
opens no database, starts no Geant4 process, and needs no definition. Rows that a
...
def main():
    ...
    ap.add_argument("--inputs", nargs="*", default=None,
                    help="confirmation JSONs to merge (default: every "
                         "results/stage4_confirmation_3200000000_primaries_*.json)")
    ap.add_argument("--registry",
                    default=os.path.join(HERE, "stage4_invalidations.yaml"))
    ap.add_argument(
        "--out",
        default=os.path.join(RESULTS, "stage4_confirmation_3200000000_primaries.json"),
    )
    ap.add_argument(
        "--compare-primary-counts",
        action="store_true",
        help="also compare the four simulated-primary counts",
    )
    args = ap.parse_args()

    paths = args.inputs or sorted(
        p for p in glob.glob(
            os.path.join(RESULTS, "stage4_confirmation_3200000000_primaries_*.json")
        )
        if os.path.abspath(p) != os.path.abspath(args.out))
    if not paths:
        sys.exit("no 3,200,000,000-event confirmation JSONs found; nothing to assemble")
    ...
    out = {"event_count": "320000000", "events": events, "seed_bank": seed_bank,
           ...
           "note": "Assembled from existing confirmation JSONs by "
                   "assemble_property_event_results.py. No simulation was run and no database "
    ...
    if args.compare_primary_counts:
        result_files = [
            os.path.join(RESULTS, f"stage4_confirmation_{n}_primaries.json")
            for n in ("4000000", "32000000", "320000000")
        ] + [args.out]
        missing = [path for path in result_files if not os.path.isfile(path)]
        if missing:
            print(f"  skipping the primary-count comparison: missing {missing}")
            return 0
        cmd = [sys.executable, os.path.join(HERE, "compare_property_event_counts.py")] \
            + result_files + [
                "--out",
                os.path.join(RESULTS, "stage4_primary_count_comparison.json"),
            ]
        print("\n" + " ".join(cmd))
