#!/usr/bin/env python3
"""Compare Stage 4 candidate rankings across event allowances.

    python compare_property_event_counts.py \\
        results/stage4_confirmation_4000000_primaries.json \\
        results/stage4_confirmation_32000000_primaries.json \\
        results/stage4_confirmation_320000000_primaries.json
...
A screening event_count is only allowed to select if its ranking survives the move to a
converged one, and that has to be measured, not assumed.

Reported per pair of event_counts:
...
def load(path):
    ...
    # objective values were unaffected (they use total events) but every event_count
    # LABEL and every event_count statement derived from them was.
    ...
    if per_sub is None:
        raise SystemExit(
            ...
            f"is how the event_count labels came to be wrong by 4x. Re-run it, or add "
            f"the field from the database:\n"
...
def main():
    ...
    ap.add_argument("event_counts", nargs="+", help="confirmation JSONs, cheapest first")
    ap.add_argument("--baseline-label", default="baseline")
    ap.add_argument("--out", default=None)
    args = ap.parse_args()

    event_counts = [load(p) for p in args.event_counts]
    event_counts.sort(key=lambda t: t["per_sub_run"])
    print("Stage 4 event_count comparison")
    for t in event_counts:
        print(f"  {t['per_sub_run']:>12,} events/sub-run "
              f"({t['events']:,}/candidate, bank {t['seed_bank']}): "
              f"{len(t['values'])} candidates  [{t['path']}]")

    # Per-candidate table across event_counts, normalised to each event_count's own baseline so
    # the comparison is of RANKINGS, not of absolute yields at different allowances.
    labels = sorted(set().union(*[set(t["values"]) for t in event_counts]),
                    key=lambda k: event_counts[-1]["values"].get(k, 9e9))
    def event_count_tag(t):
        n = t["per_sub_run"]
        return f"{n / 10 ** (len(str(n)) - 1):g}e{len(str(n)) - 1}/sub-run"

    header = f"{'candidate':28s}" + "".join(f"{event_count_tag(t):>20s}" for t in event_counts)
    print("\nQPs per primary event (relative to each event_count's own baseline in brackets):")
    print(header)
    for label in labels:
        line = f"{label[:28]:28s}"
        for t in event_counts:
    ...
    print("\nRank agreement between event_counts (baseline excluded):")
    out = {"event_counts": [{k: t[k] for k in ("path", "events", "per_sub_run", "seed_bank")}
                     for t in event_counts], "pairs": []}
    for lo, hi in itertools.combinations(range(len(event_counts)), 2):
        a, b = event_counts[lo], event_counts[hi]
    ...
    # Resolution at the finest event_count: which adjacent pairs are actually separated.
    fine = event_counts[-1]
