"""Stage 3 trial database: SQLite, append-only in spirit, restartable.

Two tables:

  trials    one row per (candidate, event_count, scenario set, seed bank)
...
# fresh database at once each issued the same ALTER TABLE; the duplicate-column
...
class DatabaseFrozen(RuntimeError):
    """The database is deliberately closed to new writers."""
...
class IncompatibleDatabase(RuntimeError):
    """A writer was pointed at an earlier schema that must remain unchanged."""


FREEZE_SUFFIX = ".frozen"


def freeze_path(database_path):
    return os.path.abspath(database_path) + FREEZE_SUFFIX


def freeze_reason(database_path):
    """Why this database is closed to new writers, or None if it is open.

    Opening a `Database` runs `_migrate()`, which ALTERs the schema, and any
    ...
    a frozen database is never even opened, and it cannot be undone by a signal --
    which is what made freezing the chain's shell unreliable.
    """
    path = freeze_path(database_path)
...
class Database:
    def __init__(self, path, allow_frozen=False, read_only=False):
        ...
        read the v2 Stage 3 database for reference values, and because opening a
        database migrates it, simply running the test suite added ten columns to a
    ...
    def _refuse_earlier_schema(self):
        """Inspect an existing file read-only before any writer opens it."""
        if not os.path.isfile(self.path):
            return
        inspection = sqlite3.connect(f"file:{self.path}?mode=ro", uri=True,
                                     timeout=60.0)
        try:
            tables = {row[0] for row in inspection.execute(
                "SELECT name FROM sqlite_master WHERE type='table'"
            )}
            if "trials" not in tables:
                return
            columns = {row[1] for row in inspection.execute("PRAGMA table_info(trials)")}
        finally:
            inspection.close()
        required = {"definition_hash", "event_count"}
        missing = sorted(required - columns)
        if missing:
            raise IncompatibleDatabase(
                f"{self.path} uses an earlier result schema (missing {missing}) and "
                "will not be opened by a writer. Inspect it with "
                "`python -m material_scan history-summary`, or choose a new SQLite "
                "path for the revised experiment."
            )

    # Columns added after the first campaigns. An existing database is migrated in
    ...
    _ADDED_COLUMNS = {
        ...
        "trials": (("objective_name", "TEXT"), ("objective_value", "REAL"),
                   ...
                   # Finding N2: `definition_hash` is the CAMPAIGN identity (it
    ...
    def _migrate(self, locked=False):
        ...
        fresh database at once each saw the column missing and each issued the
        ...
        was reachable on the first open of any new database.
    ...
    def plan_trial(self, trial_id, campaign_id, cache_key, definition_hash,
                   code_fingerprint, candidate, derived, event_count, events_total,
    ...
    def set_optimizer_record(self, trial_id, objective_name=None, objective_value=None,
        ...
        """Optimizer bookkeeping (pipeline sec 7's database fields).
    ...
    def observations(self, campaign_id=None, include_controls=False):
        ...
        # A read-only historical database is never migrated, so the column may not
        # exist -- and a database with no controls column has no controls to
...
if __name__ == "__main__":
    ...
    with Database(path) as database:
        print(f"Database at {database.path}")
        print("  trials by status:", database.summary() or "(empty)")
