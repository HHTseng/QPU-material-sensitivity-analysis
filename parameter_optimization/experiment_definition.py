"""Stage 3 experiment definition: load, validate, and hash.
...
The definition also carries the code identity that goes into every cache key: if
...
Usage:
    ...
    definition = load_definition("stage3_config.yaml")
    definition.check_excitation_thresholds()
    print(definition.code_fingerprint())
"""
...
class DefinitionError(ValueError):
    """Raised when the definition is incomplete, ambiguous, or self-inconsistent."""
...
class Definition:
    ...
    def validate(self):
        ...
        for name, spec in self.decision.items():
            if not isinstance(spec, dict):
                raise DefinitionError(f"decision.{name} must be a mapping, got {type(spec).__name__}")
            ...
            if value_key not in spec:
                raise DefinitionError(f"decision.{name} declares no `{value_key}`.")
            if allowed_key is not None:
                ...
                # `space` section and is hashed with the definition.
    ...
    # -- event accounting ---------------------------------------------------
    def events_per_sub_run(self, event_count=None):
        ...
        """
        event_count = event_count or self.decision["event_count"]["value"]
        total = int(self.decision["event_count"]["events_total_per_candidate"][event_count])
        sub_runs = int(self.fixed["n_positions"]) * int(self.fixed["n_replicas"])
        if total % sub_runs != 0:
            raise DefinitionError(
                f"event_count {event_count}: {total:,} events do not divide evenly into "
    ...
    #
    # `definition_hash()` used to be both "which campaign is this" and "would a
    ...
    # changing the primary-count comparison from 32 to 64 workers created new cache keys and
    ...
    #
    #   campaign_definition_hash()   -- the whole declaration, for provenance and
    ...
    def campaign_definition_hash(self):
    ...
    def simulation_identity_hash(self):
        ...
        search/scoring metadata. Everything else in the definition is included,
    ...
    def definition_hash(self):
        ...
        Kept because it is what the database's `definition_hash` column has always
        held and what `resume()` compares against. New code should say which
        identity it means.
        """
        return self.campaign_definition_hash()
...
def load_definition(path):
    ...
    if missing:
        raise DefinitionError(f"{path}: missing required section(s) {missing}")
    if "campaign_id" not in raw:
        raise DefinitionError(f"{path}: missing campaign_id")
...
if __name__ == "__main__":
    ...
    c = load_definition(target)
    print(f"Definition OK: {c.campaign_id}")
    print(f"  fixed={len(c.fixed)} decision={len(c.decision)} derived={len(c.derived)}")
    print(f"  campaign_definition_hash  ={c.campaign_definition_hash()[:16]}")
    ...
    for fid in c.decision["event_count"]["allowed"]:
