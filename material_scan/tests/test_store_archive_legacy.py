...
class LegacyTests(unittest.TestCase):
    ...
    def test_reader_never_migrates_and_reports_foreign_key_orphan(self) -> None:
        ...
        before_columns = sqlite3.connect(database).execute(
            "PRAGMA table_info(trials)"
        ).fetchall()
        with LegacyLedger(database, static_snapshot=True) as ledger:
            self.assertEqual([row["trial_id"] for row in ledger.trials()], ["complete"])
            self.assertEqual(len(ledger.integrity_report()["foreign_key_violations"]), 1)
            with self.assertRaises(sqlite3.OperationalError):
                ledger._connection.execute("ALTER TABLE trials ADD COLUMN forbidden TEXT")
        self.assertEqual(sha256_file(database), before_hash)
    ...
    def test_logical_backup_is_explicit_atomic_and_source_preserving(self) -> None:
        ...
        with LegacyLedger(database, static_snapshot=True) as ledger:
            backup = ledger.backup_to(self.root / "snapshots" / "ledger.sqlite")
        self.assertEqual(sha256_file(database), original)
    ...
    def test_absolute_path_remap_refuses_prefix_and_traversal(self) -> None:
        ...
        for value in ("/old/root-other/file", "/old/root/../escape", "relative/file"):
            with self.subTest(value=value), self.assertRaises(UnsafeLegacyPath):
