...
Nothing in this module imports the historical database implementation.  That is
intentional: constructing the old ``Database`` class can create tables and run
...
class HistoricalError(RuntimeError):
    """A historical source is unsafe, inconsistent, or unsupported."""


class UnsafeHistoricalPath(HistoricalError):
...
def _inventory_hash(path: Path, expected: os.stat_result) -> str:
    ...
    try:
        ...
        if signature(opened) != signature(expected) or not stat.S_ISREG(opened.st_mode):
            raise HistoricalError(f"file changed while inventorying: {path}")
        digest = hashlib.sha256()
...
def _safe_relative(path: str) -> str:
    if not path or "\x00" in path or "\\" in path:
        raise UnsafeHistoricalPath(f"unsafe relative path: {path!r}")
    if any(part in ("", ".", "..") for part in path.split("/")):
        raise UnsafeHistoricalPath(f"non-canonical relative path: {path!r}")
...
def remap_absolute_path(stored_path: str, recorded_root: os.PathLike[str] | str,
    ...
    if not os.path.isabs(stored_path) or not os.path.isabs(os.fspath(recorded_root)):
        raise UnsafeHistoricalPath("stored path and recorded root must both be absolute")
    ...
    try:
        ...
        raise UnsafeHistoricalPath("mapped path escapes the actual root") from exc
    return candidate


class HistoricalDatabase:
    """A historical SQLite reader which cannot create or migrate schema."""

    def __init__(self, path: os.PathLike[str] | str, *, static_snapshot: bool = False):
        self.path = Path(path).resolve()
        if not self.path.is_file():
            raise HistoricalError(f"historical database does not exist: {self.path}")
        wal = Path(str(self.path) + "-wal")
        if static_snapshot and wal.exists() and wal.stat().st_size:
            raise HistoricalError("immutable mode would ignore the non-empty WAL")
    ...
    def columns(self, table: str) -> tuple[str, ...]:
        if table not in self._tables:
            raise HistoricalError(f"missing historical table: {table}")
    ...
    def backup_to(self, destination: os.PathLike[str] | str) -> Path:
        ...
        if target.exists():
            raise HistoricalError(f"backup destination already exists: {target}")
        ...
        try:
            ...
            try:
                ...
                if output.execute("PRAGMA integrity_check").fetchone()[0] != "ok":
                    raise HistoricalError("logical SQLite backup failed integrity_check")
            finally:
                output.close()
            ...
            try:
                ...
                raise HistoricalError(f"backup destination appeared: {target}") from exc
            temporary.unlink()
...
class GitTree:
    ...
    def __init__(self, repository: os.PathLike[str] | str,
        ...
        if not (self.repository / ".git").exists():
            raise HistoricalError(f"not a Git work tree: {self.repository}")
        if not _REVISION.fullmatch(revision) or revision.startswith("-") or ".." in revision:
            raise HistoricalError(f"unsafe Git revision: {revision!r}")
    ...
    def _run(self, arguments: Sequence[str], *, binary: bool = True) -> bytes | str:
        ...
        if result.returncode:
            ...
            raise HistoricalError(f"git {' '.join(arguments[:2])} failed: {message}")
    ...
    def blob_metadata(self, object_ids: Iterable[str]) -> Mapping[str, tuple[int, str]]:
        ...
        if any(not re.fullmatch(r"[0-9a-fA-F]+", value) for value in identifiers):
            raise HistoricalError("Git object IDs must be hexadecimal")
        environment = dict(os.environ)
        environment["GIT_OPTIONAL_LOCKS"] = "0"
        ...
        try:
            for object_id in identifiers:
                ...
                if len(fields) != 3 or fields[1] != "blob":
                    raise HistoricalError(f"Git object is not a readable blob: {header}")
                size = int(fields[2])
                remaining = size
                digest = hashlib.sha256()
                while remaining:
                    ...
                    if not block:
                        raise HistoricalError("git cat-file ended inside a blob")
                    remaining -= len(block)
                    digest.update(block)
                if process.stdout.read(1) != b"\n":
                    raise HistoricalError("git cat-file blob separator is malformed")
            ...
            if return_code:
                raise HistoricalError(stderr.decode("utf-8", "replace").strip())
            return output
        except BaseException:
            try:
                process.stdin.close()
...
def classify_historical_artifact(path: str) -> str:
    ...
    for directory, dir_names, file_names in os.walk(root, followlinks=False):
        ...
        for name in tuple(dir_names):
            ...
            if candidate.is_symlink():
                ...
                records[relative] = InventoryRecord(
                    relative, classify_historical_artifact(relative), "symlink",
        ...
        for name in file_names:
            ...
            if stat.S_ISREG(info.st_mode):
                ...
                digest = None
            records[relative] = InventoryRecord(
                relative, classify_historical_artifact(relative), file_type,
...
def build_inventory(root: os.PathLike[str] | str, *,
    ...
    if not source.is_dir() or source.is_symlink():
        raise HistoricalError(f"inventory root must be a real directory: {source}")
    ...
    if git_tree is not None:
        ...
        for entry in entries:
            ...
            if present is None:
                records[relative] = InventoryRecord(
                    relative, classify_historical_artifact(relative),
...
def read_inventory_sqlite(path: os.PathLike[str] | str) -> Inventory:
    ...
    if not source.is_file():
        raise HistoricalError(f"inventory does not exist: {source}")
    ...
    try:
        ...
        if version != _INVENTORY_SCHEMA_VERSION:
            raise HistoricalError(f"unsupported inventory schema version: {version}")
        metadata = dict(connection.execute("SELECT key,value FROM metadata"))
        if (set(metadata) != {"schema", "root", "digest", "records"}
                or metadata["schema"] != "material-scan-historical-inventory-1"):
            raise HistoricalError("inventory metadata schema is malformed")
        ...
        if int(metadata["records"]) != len(records) or metadata["digest"] != inventory.digest:
            raise HistoricalError("inventory metadata does not match its artifact rows")
        return inventory
    except (KeyError, TypeError, ValueError, sqlite3.Error) as exc:
        raise HistoricalError(f"malformed inventory database: {source}") from exc
    finally:
        connection.close()
...
def write_inventory_sqlite(inventory: Inventory,
    ...
    try:
        ...
        raise HistoricalError("inventory database must be outside the inventoried tree")
    if target.exists():
        existing = read_inventory_sqlite(target)
        if existing.digest != inventory.digest or existing.root != inventory.root:
            raise HistoricalError(f"inventory destination has different content: {target}")
        return target
    target.parent.mkdir(parents=True, exist_ok=True)
    descriptor, temporary_name = tempfile.mkstemp(
        prefix=f".{target.name}.", suffix=".partial", dir=target.parent
    ...
    try:
        ...
        connection.executemany(
            ...
            (("schema", "material-scan-historical-inventory-1"),
             ("root", inventory.root),
        ...
        if connection.execute("PRAGMA integrity_check").fetchone()[0] != "ok":
            raise HistoricalError("new inventory failed SQLite integrity_check")
        connection.close()
        connection = None
        with open(temporary, "rb") as handle:
            os.fsync(handle.fileno())
        try:
            ...
            existing = read_inventory_sqlite(target)
            if existing.digest != inventory.digest or existing.root != inventory.root:
                raise HistoricalError(f"inventory destination appeared with different content: {target}")
            temporary.unlink()
            return target
        temporary.unlink()
        _fsync_directory(target.parent)
