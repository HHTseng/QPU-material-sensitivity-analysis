...
MACRO_TEMPLATE = os.path.join(SCRIPT_DIR, "sensitivity_template.mac")
CONFIG_TEMPLATE = os.path.join(G4CMP_SOURCE_CRYSTALMAPS, "Si", "config.txt")

RUN_ID = "percentage_" + unique_id
...
for path in (HITS_DIR, MACROS_DIR, CRYSTALMAPS_DIR):
...
lbf = 1.00 # lower bound factor
ubf = 1.00 # upper bound factor

#BNL_G4CMP electrode parameters. Format is (macro command, default value, range, unit (optional)).
electrode_params = [
#("/main/gun/setEnergy ", 0.000382, [0.000382*lbf,0.000382*ubf], " eV"), # Note that later command sets the phonon injection energy to scale with the gap energy of the Al junctions
...
("/main/detector_param/setTopAbs ", 0.795, [0.795*lbf,0.795*ubf]),
...
#("/main/detector_param/setTopQPLim ", 3, [3,3], "int"),
...
("/main/detector_param/setTopPSpecProb ", 0.0, [0,0.05]),
#("/main/detector_param/setTopFilmSourceMat ", "G4_Nb")
("/main/detector_param/setTopFilmAbs ", 0.745, [0.745*lbf,0.745*ubf]),
...
#("/main/detector_param/setTopFilmQPLim ", 3, [3,3], "int"),
...
("/main/detector_param/setTopFilmPSpecProb ", 0.0, [0,0.05]),
#("/main/detector_param/setBotSourceMat ", "G4_Cu")
("/main/detector_param/setBotAbs ", 0.736, [0.736*lbf,0.736*ubf]),
...
#("/main/detector_param/setBotQPLim ", 2, [2,2], "int"),
...
("/main/detector_param/setBotPSpecProb ", 0.0, [0,0.05]),
("/main/detector_param/setWallAbs ", 0.025, [0.025*lbf,0.025*ubf]),
("/main/detector_param/setWallPSpecProb ", 0.0, [0,0.05]),
...
#"/g4cmp/LatticeData ",
#"/g4cmp/verbose ",
#("/g4cmp/clearance ", 1e-06, [1*lbf,1*ubf], "e-6 mm"),
#"/g4cmp/voltage ",
#"/g4cmp/EPotFile ",
#"/g4cmp/scaleEPot ",
#"/g4cmp/minimumStep ",
#("/g4cmp/chargeBounces ", 1, [1,1], "int"),
#("/g4cmp/phononBounces ", 1000, [1000*lbf,1000*ubf], "int"),
...
#("/g4cmp/eTrappingMFP ", 0.3, [0.3*lbf,0.3*ubf], " mm"),
#("/g4cmp/hTrappingMFP ", 0.3, [0.3*lbf,0.3*ubf], " mm"),
...
#("/g4cmp/temperature ", 0.0, [0.0*lbf, 0.0*ubf], " K"),
...
("decayTT ", 0.74, [0.74*lbf,0.74*ubf]),
...
("acDeform ", 6.6, [6.6*lbf,6.6*ubf], " eV"),
...
("s", [0.02808,0.0523,0.0292,0.0371,0.0331,0.03038], [[0.02808*lbf,0.02808*ubf], [0.0523*lbf,0.0523*ubf], [0.0292*lbf,0.0292*ubf], [0.0371*lbf,0.0371*ubf], [0.0331*lbf,0.0331*ubf], [0.03038*lbf,0.03038*ubf]]),
#("s", 0.05, [0.01,0.1]),
("I_ph", 1.67259*6.7578e5, [1.67259*6.7578e5*lbf,1.67259*6.7578e5*ubf]),
...
for index, param in enumerate(all_params):
    if param[0] == "/main/detector_param/setTopGap ":
        topgap_index = index

    if type(param[1]) is not list:
        lowerbound.append(param[2][0])
        upperbound.append(param[2][1])
        names.append(param[0])
    else:
        n = 1
        for p in param[2]:
            lowerbound.append(p[0])
            upperbound.append(p[1])
            names.append(param[0] + "_" + str(n))
            n += 1
            
                    
means = np.array(lowerbound)/lbf
d = len(means)
covmat = np.zeros((d,d))
for i in range(d):
    covmat[i,i] = (0.00*means[i])**2

sampler = qmc.MultivariateNormalQMC(mean=means, cov=covmat, seed=4)
samples = np.round(sampler.random(n=100), 7)

def replace_line(lines,start_string,new_line):
...
with open(os.path.join(RESULTS_DIR, "0perc_Normal.csv"), 'w', newline='') as csvfile:
...
def generate_runfiles(samp,samp_No):
    ...
    sample_name = "0perc_Normal_" + str(samp_No)
    ...
    lines = replace_line(lines, "/main/detector_param/setSubstrateName ","/main/detector_param/setSubstrateName 0perc_Normal_" + str(samp_No))
    lines = replace_line(lines, "/main/gun/setEnergy ", "/main/gun/setEnergy " + str(2*samp[topgap_index])) # replace the phonon energy with twice the topgap energy.
    ...
    output_config_dir = os.path.join(CRYSTALMAPS_DIR, sample_name)
...
def run_sample(macroname):
    ...
    command = (
        ...
        + "set -u; "
        + "export G4LATTICEDATA=" + shlex.quote(CRYSTALMAPS_DIR) + "; "
...
def main(macro_files):
    with ProcessPoolExecutor(max_workers=128) as executor:
        executor.map(run_sample, macro_files)

if __name__ == "__main__":
    macro_files = []
    for s, samp in enumerate(samples):
        generate_runfiles(samp,s)
        macro_files.append(os.path.join(MACROS_DIR, "0perc_Normal_" + str(s) + ".mac"))
